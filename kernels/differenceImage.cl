...
kernel void memset(   global unsigned char *left_src, global unsigned char *right_src, global unsigned char *dst, int height, int width, int max_dist)
/*!
...
{   const int x = get_global_id(0);
    ...
    const int input_id = (x + (y*(width+2*max_dist)) ) + max_dist ;
    ...
    for(int i = 0; i<max_dist; i++){ // compare with pixel at the same position until pixel at max_dist-1
        ...
        const int output_id = x + (y*((width +2*max_dist)*max_dist)) + max_dist + i*(width + 2*max_dist);
