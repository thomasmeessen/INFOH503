kernel void memset(__global unsigned char* src, __global double* dst_a_k, __global double* dst_b_k, __global double* cost, int original_width, int original_height, int max_dist) {
    ...
    int radius = 2;
    double omega_size = 25;
    int src_image_pixels_sum = 0;
    int src_image_pixels_sum_square = 0;
    double p_k_sum = 0;
    double p_pixels_product = 0;
    double epsilon = (255*255)/10000;
    ...
    for (int i = -radius; i <= radius; i++) {
        for (int j = -radius; j <= radius; j++) {
            ...
            double cost_pixel = cost[cost_id];
            src_image_pixels_sum += source_pixel;
            src_image_pixels_sum_square += (source_pixel * source_pixel);
            p_k_sum += cost_pixel;
            p_pixels_product += (double) source_pixel *  cost_pixel;
            //printf("%f \n", cost[id]);
        }
    }

    double mu_k = src_image_pixels_sum / omega_size;
    double sigma_k_square = (src_image_pixels_sum_square / omega_size) - (mu_k * mu_k);
    double p_k = p_k_sum / omega_size;

    double a_k = ((p_pixels_product / omega_size) - mu_k * p_k) / (sigma_k_square + epsilon);
    double b_k = p_k - a_k * mu_k;
